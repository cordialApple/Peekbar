...
Reference for `TaskbarOverlayWindow` (Stage 5b). ALL heuristics are isolated to
that one file (hard rule 6). This doc records *why* the code looks the way it
does, so a future Windows update that breaks it is a one-file plus one-doc fix.
...
The Win10 per-button `MSTaskListWClass` is **absent** on Win11.

The `MSTaskSwWClass` ("Running applications") container HWND still exists,
**but its rect is a legacy stub that does NOT match the XAML layout.**

Measured live on build 26200 @ 200%: `MSTaskSwWClass` = 45 to 577 (in the
non-PMv2 probe's virtualized coords), while the actual app icons render out to
~1418. A pure-HWND measurement lands badly wrong on Win11.

**UIA is required** to read the real XAML element rects. Confirmed the hard
way: the first HWND-only 5b.1 outline spanned far too wide on both sides.
...
buttons end at `..1418`; `WidgetsButton` = `1948..2252`; tray starts 2276. So
gap = **[1418, 1948]**.

In a PMv2 process, UIA `BoundingRectangle` and `GetWindowRect` share one
physical-pixel space (no conversion). The 1440-vs-2880 discrepancy seen in a
raw PowerShell probe is only because that probe wasn't PMv2.

Guards (each prevents a visibly-wrong outline): explorer.exe owner verify;
`IsAutoHide` bail; null `TrayNotifyWnd` invalidates; `GetDpiForWindow(tray)==0`
invalidates; no task-button matched (`gap.left == tray.left`) invalidates;
min-gap threshold (scaled by the taskbar monitor's DPI).

Debt: the overflow/"show more" chevron's class is not confirmed. If the task
list overflows, it may not be counted as a task button, and the outline could
overlap it. Revisit with a live overflowed taskbar.
...
2. **Sleep-wake stale rect.** After resume, `MSTaskSwWClass`'s rect can be stale
   ...
   not sane, skip this measurement (retry on next tick or delayed remeasure).
3. **Auto-hide.** `SHAppBarMessage(ABM_GETSTATE)` & `ABS_AUTOHIDE`: taskbar is
   a 1 to 2px sliver, so hide the overlay. (`ABM_GETSTATE` ignores `hWnd`;
   null is fine.) This is a synchronous `SendMessage` to explorer on the UI
   thread. That's fine as a light query, but 5b.3 should fence it
   (`SendMessageTimeout`) so a hung explorer can't hold the pump.
...
- `HKCU\...\Explorer\Advanced\TaskbarAl`: 0 = left-aligned, 1 = centered (Win11
  ...
  Start/Search/Widgets/TaskView. Not clean, so we only use the right-side gap.
- As apps open/close, `MSTaskSwWClass` grows/shrinks, so the right-side gap
  tracks it. Fall back to 5a dock-hosted buttons when gap < min threshold.
